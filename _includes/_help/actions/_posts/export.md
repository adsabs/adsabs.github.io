...
### The {{ include.site }} Custom Format

The custom format feature allows you to output {{ include.site }} data in a completely user-defined format. For example, the following format
...
Note that formats %e and %f used to be known in Classic {{ include.site }} as %za2 and %za3 respectively. %za1 from Classic {{ include.site }} is deprecated since it is equivalent to %1H.
...
**u** | URL | prints out the URL of the abstract in the form: {% if include.ads %}{{ site.ads_base_url }}{% else %}{{ site.scix_base_url }}{% endif %}/#abs/bibcode/abstract
