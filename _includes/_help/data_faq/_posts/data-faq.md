...
* [Missing articles/corrections to existing article]({% if include.ads %}{{ site.ads_base_url }}{% else %}{{ site.scix_base_url }}{% endif %}/feedback/correctabstract)
...
Note that these forms do not work if you're using the basic HTML version of {{ include.site }}, and occasionally, some users encounter errors when submitting these forms; [email us](mailto:adshelp@cfa.harvard.edu) if you run into issues.
...
We receive author names and other metadata directly from the publisher. First check that your name is correct on the published article; if it isn't, first contact the publisher directly to fix it with them. However, if your name is correct with the publisher and wrong in the {{ include.site }} database, [fill out this form]({% if include.ads %}{{ site.ads_base_url }}{% else %}{{ site.scix_base_url }}{% endif %}/feedback/correctabstract) and we'll correct it.
...
* You may also consider [registering for an ORCID ID]({% if include.ads %}{{ site.ads_base_url }}{% else %}{{ site.scix_base_url }}{% endif %}/blog/orcid-claims) and using that to maintain a personal bibliography.
...
For more information, see our [name change policy]({% if include.ads %}{{ site.ads_base_url }}{% else %}{{ site.scix_base_url }}{% endif %}/help/policies/name-changes).

##### Q: What journals and other material do you index?
Complete lists of the journals, conferences, and other sources we index may be found [here]({% if include.ads %}http://adsabs.harvard.edu/abs_doc/journal_abbr.html{% else %}{{ site.scix_base_url }}/help{% endif %}).
...
Full instructions for submitting conference proceedings can be found [here]({% if include.ads %}{{ site.ads_base_url }}{% else %}{{ site.scix_base_url }}{% endif %}/help/data_faq/conf_proc).
...
Submit your corrections using [this form]({% if include.ads %}{{ site.ads_base_url }}{% else %}{{ site.scix_base_url }}{% endif %}/feedback/correctabstract).
...
If it's right on the publisher's website but wrong on ours, we're sorry for the error! [Contact us]({% if include.ads %}{{ site.ads_base_url }}{% else %}{{ site.scix_base_url }}{% endif %}/feedback/correctabstract) so we can fix it.
...
3. Once your paper has been published and is indexed in {{ include.site }}, {{ include.site }} will recognize that the bibliography contains a software record and will automatically create a corresponding software record in the {{ include.site }} database. The publisher will be listed as Zenodo and the bibstem (the abbreviation that appears in the bibcode) will be [zndo]({% if include.ads %}{{ site.ads_base_url }}{% else %}{{ site.scix_base_url }}{% endif %}/#search/q=bibstem%3A%22zndo%22&sort=date%20desc%2C%20bibcode%20desc&p_=0). The publication type for these records is [software]({% if include.ads %}{{ site.ads_base_url }}{% else %}{{ site.scix_base_url }}{% endif %}/#search/q=doctype%3Asoftware&sort=date%20desc%2C%20bibcode%20desc&p_=0).
