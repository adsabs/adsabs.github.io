...
Note there is also `%zm` specifier that is used for enumeration. This format outputs sequence of characters to the end of `keyformat` when it is not unique among selected records. For example having `keyformat` of `%1H%Y%zm` produces:
```
Accomazzi2019a
Accomazzi2019b
Kurtz2019
...
**pc** | Page Count | prints out the number pages of the article if available
