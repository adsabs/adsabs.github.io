...
* **Z**: Command format 
    ...
    There are two encoding options: global and field. As the name suggests specifying global encoding applies the encoding to the entire citiation, all output field. Please see further down for explanation of field encoding.
    * **Encoding:word** Available global encoding parameters are: 
...
* **Field encoding** is applied to each individual field. Available field encoding characters are:
    ...
    * **\\**: Tex/Latex encoding. This encoding converts all special Tex characters into their Tex escape sequences. For instance '\' is converted to '$\backslash $', '$' is converted into '\$', '^' is converted into '\^{}', etc.
