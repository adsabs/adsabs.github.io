...
Note there is also `%zm` specifier that is used for enumerating. This format outputs sequence characters when `keyformat` is not unique among selected records.
