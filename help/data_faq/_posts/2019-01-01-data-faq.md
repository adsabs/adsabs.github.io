...
For now, use the forms from ADS Classic, linked below and also from the Feedback menu on the search page; these will be ported to the current ADS soon and the links will be updated. Note that these forms do not work if you're using the basic HTML version of ADS, and occasionally, some users encounter errors when submitting these forms; [email us](mailto:adshelp@cfa.harvard.edu) if you run into issues.  
