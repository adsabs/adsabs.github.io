...
For now, use the forms linked below from ADS Classic; these will be ported to the new ADS soon and the links will be updated. Note that some users occasionally encounter errors when submitting these forms; [email us](mailto:adshelp@cfa.harvard.edu) if you run into issues.  
...
##### Q: Do Classic and the new ADS use the same database?
No, Classic and the new ADS use different databases, though we work to ensure they contain the same content. Currently, database updates in the new ADS are a few hours behind those in Classic, though this lag is improving.
